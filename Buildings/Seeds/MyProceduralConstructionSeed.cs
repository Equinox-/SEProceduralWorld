...
namespace Equinox.ProceduralWorld.Buildings.Seeds
{
    public class MyProceduralConstructionSeed
    {
        ...
        public MyProceduralConstructionSeed(Vector4D locationDepth, Quaternion? orientation, long seed)
        {
            ...
            Population = (int)MyMath.Clamp((float)(Math.Round(5 * Random.NextExponential() * density)), 1, 100);
            ...
            Speciality = choice.Choose(Random.NextDouble(), MyWeightedChoice<MyProceduralStationSpeciality>.WeightedNormalization.ClampToZero);
            m_exports = new Dictionary<MyDefinitionId, MyTradeRequirements>();
            m_imports = new Dictionary<MyDefinitionId, MyTradeRequirements>();
            List<MyDefinitionBase> rchoice = null;
            var specialExport = Random.NextDouble() <= Speciality.SpecializationChance;
            if (specialExport)
            ...
            if (specialExport)
            {
                ...
                SpecialityExport = rchoice[Random.Next(0, rchoice.Count)].Id;
                m_exports.Clear();
                m_exports.Add(SpecialityExport, default(MyTradeRequirements));
            }

            var nameBuilder = new StringBuilder();
            nameBuilder.Append(Faction.FounderName).Append("'s ");
            if (specialExport)
                nameBuilder.Append(MyDefinitionManager.Static.GetDefinition(SpecialityExport).DisplayNameText).Append(" ");
            else if (Speciality.GeneralizedPrefixes != null && Speciality.GeneralizedPrefixes.Length > 0)
                nameBuilder.Append(Random.NextUniformChoice(Speciality.GeneralizedPrefixes)).Append(" ");
            nameBuilder.Append(Random.NextUniformChoice(Speciality.Suffixes));
            ...
            // Compute hotlisted import amounts
            {
                ...
                {
                    var baseMult = Population * Random.NextExponential() / keys.Count;
            ...
            // Compute exported amounts
            {
                ...
                foreach (var kv in keys)
                {
                    var baseMult = Population * Population * Random.NextExponential() / keys.Count;
                    var k = MyDefinitionManager.Static.GetDefinition(kv);
                    var pi = k as MyPhysicalItemDefinition;
                    if (k is MyComponentDefinition)
                        baseMult *= 100 / pi.Mass;
            ...
            m_imports = new Dictionary<MyDefinitionId, MyTradeRequirements>(m_exports.Count * 2);
            // Using exports, compute imports
            {
                foreach (var kv in m_exports)
                {
                    var producer = MyBlueprintIndex.Instance.GetAllProducing(kv.Key).ToList();
            ...
            m_localStorage = new Dictionary<MyDefinitionId, MyTradeRequirements>
            {
                // ~3MWH / person stored, ~1MW / person produced
                [MyResourceDistributorComponent.ElectricityId] = new MyTradeRequirements(Population * 3 * Random.NextNormal(1, 0.1), Population * Random.NextNormal(1, 0.1), 10, 10),
                // one large O2 tank / person stored (~18 days), 1 O2/sec/person produced
                [MyResourceDistributorComponent.OxygenId] = new MyTradeRequirements(Population * 1e5 * Random.NextNormal(1, 0.1), Population * 1 * Random.NextNormal(1, 0.1), 10, 10),
                // I don't even know how I'd guess this
                [MyResourceDistributorComponent.HydrogenId] = new MyTradeRequirements(Population * 1e4 * Random.NextExponential(), Random.NextExponential() * sqrtPopulation)
            };
            ...
            m_blockCountRequirements[MySupportedBlockTypes.CryoChamber] = Population + Math.Max(0, (int)Math.Round(sqrtPopulation * (Random.NextNormal(1, 2) + addlLiving)));
            m_blockCountRequirements[MySupportedBlockTypes.MedicalRoom] = new MyBlockRequirement(Math.Max(1, (int)Math.Round(sqrtPopulation * (Random.NextNormal(0.5, 0.5) + Math.Max(addlLiving, Faction.AttributeWeight(MyProceduralFactionSpeciality.Military))))), 1e3);
            ...
            m_blockCountRequirements[MySupportedBlockTypes.Weapon] = Math.Max(0, (int)Math.Round(Population * Random.NextNormal(2, 2) * Faction.AttributeWeight(MyProceduralFactionSpeciality.Military)));
            // ship repair?
            m_blockCountRequirements[MySupportedBlockTypes.ShipConstruction] = Math.Max(0, (int)Math.Round(Population * Random.NextNormal(4, 3) * Faction.AttributeWeight(MyProceduralFactionSpeciality.Repair)));
            // docking?
            m_blockCountRequirements[MySupportedBlockTypes.Docking] = Math.Max(1, (int)Math.Round(sqrtPopulation * Random.NextNormal(1, 1) * Faction.AttributeWeight(MyProceduralFactionSpeciality.Housing)));
            // comms?
            m_blockCountRequirements[MySupportedBlockTypes.Communications] = new MyBlockRequirement(Math.Max(1, (int)Math.Round(sqrtPopulation * MyMath.Clamp((float)Random.NextNormal(), 0, 1))), 1e6);

            Orientation = orientation ?? Random.NextQuaternion();
        }
        ...
        public struct MyTradeRequirements
        {
            ...
            public MyTradeRequirements(double storage, double throughput, double storageErrMult = 1, double throughErrMult = 1)
            {
                ...
                StorageErrorMultiplier = storageErrMult;
                ThroughputErrorMultiplier = throughErrMult;
            }
        }
