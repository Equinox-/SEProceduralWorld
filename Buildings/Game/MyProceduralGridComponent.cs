...
namespace Equinox.ProceduralWorld.Buildings.Game
{
    ...
    public class MyProceduralGridComponent : MyEntityComponentBase
    {
        ...
        private static void ApplyResurive(MyHierarchyComponentBase e, Action<IMyEntity> apply)
        {
            if (e.Entity != null)
                apply.Invoke(e.Entity);
            foreach (var k in e.Children)
                ApplyResurive(k, apply);
        }
        ...
        public bool IsConcealed
        {
            ...
            set
            {
                ...
                if (value)
                {
                    ...
                    {
                        MyGamePruningStructure.Remove((MyEntity)grid);
                        ApplyResurive(grid.Hierarchy, x =>
                        {
                            MyAPIGateway.Entities.UnregisterForUpdate(x);
                            x.Physics?.Deactivate();
                        });
                    }
                }
                else
                {
                    foreach (var grid in m_grids)
                    {
                        ...
                        MyGamePruningStructure.Add((MyEntity)grid);
                        ApplyResurive(grid.Hierarchy, x =>
                        {
                            MyAPIGateway.Entities.RegisterForUpdate(x);
                            x.Physics?.Activate();
                        });
                    }
                }
